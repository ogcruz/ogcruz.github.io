 ...
 ggplot(iris, aes( Species,Sepal.Length)) + geom_boxplot()  + theme_linedraw()
 ...
 ggplot(iris.wide,aes(tipo,valor,fill=Species)) + geom_boxplot()
  ...
  ggplot(iris.wide,aes(tipo,valor,fill=tipo)) + 
    geom_boxplot() +
    facet_grid(~Species) +
    theme(axis.text.x=element_text(angle=90,hjust=1),legend.position="none",panel.background =element_rect(fill="white",color="black") )



  ggplot(iris, aes(x = Sepal.Length,color=Species,fill=Species)) + geom_density(alpha=0.5) + theme_minimal()
