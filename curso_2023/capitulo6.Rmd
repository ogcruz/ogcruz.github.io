...
# Simulando um processo espacial aleatório
...
# o comando jitter cria uma flutuação aleatória sobre os valores
# o comando gridpts gera uma grade regular
# 2 é o fator de confusão adicionado ao grid regular
```
...
# Analisando o efeito de primeira ordem: variação da intensidade
# kernel quartico
...
O valor de **h** que apresenta o menor Erro Quadrático Médio (MSE) é de `r Mse2d$h[which(Mse2d$mse == min(Mse2d$mse))]`.

<!-- Oswaldo: Fazer figura com valor sugerido -->

```{r echo=T, warning=FALSE, results='asis', message = FALSE, out.width = "100%", fig.align = "center"}

clu.ker.mse <- kernel2d(clu.spp,caixa,h0=0.09,nx=100,ny=100)

par(mfrow=c(1,2))
image(clu.ker.mse, col=gray(32:0/32), main="Cluster - Tau = 0.09")
pointmap(clu.spp, add=T,pch=19,col=2,cex=0.4)
contour(clu.ker.mse,add=T,levels = seq(0,500,by=100))
