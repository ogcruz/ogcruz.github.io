...
## O que são de Estudos Ecológicos ?
...
![](figuras/ecologico2.png)


<small> Figura: Correlation between per capita meat consumption and colon cancer among women in various countries (From ARMSTRONG BK & DOLL R, Environmental factors and cancer incidence and mortality in different countries, with special reference to dietary practices. Int. J. Cancer, 15: 617, 1975)
</small>
...
```{r, echo=FALSE,message=FALSE,warning=FALSE,error=FALSE, fig.align='center', fig.height=4}

renda <- c(10.5, 34.5, 28.5, 12.2, 45.6, 17.5, 19.8,
           12.5, 32.5, 24.3, 10, 14.3, 38, 26.4,
...
casos <- c(1,0,0,1,0,1,1,
           1,0,0,1,1,0,0,
           0,0,1,0,1,0,0)

tabela <- data.frame(renda, pop, casos)
...
p <- ggplot(tabela,
       aes(x = pop, y = renda, fill = factor(pop))) +
 geom_boxplot(alpha=0.3) +
    theme(legend.position="none") +
  labs(x = "Populações", y = "Renda per capita") +
...
p1 <- tabela %>%
  ...
  summarise(renda_media = mean(renda,na.rm=TRUE), 
            tx_acidente =   round(sum(casos)/n()*100, 2))
...
ggplotRegression(lm(tx_acidente ~ renda_media, data = p1))
...
DT::datatable(t2, filter="top", options = list(pageLength = 5, scrollX=T))
...
p <- ggplot(t2,
       aes(x = casos, y = renda, fill = factor(casos))) +
   geom_boxplot(alpha=0.3) +
    theme(legend.position="none") +
  labs(x = "Acidente", y = "Renda per capita") +
stat_summary(fun.y = mean, color = "darkred", position = position_dodge(0.75),
             geom = "point", shape = 19, size = 3,
             show.legend = FALSE) +
  theme_light()
...
```{r, echo=FALSE,message=FALSE,warning=FALSE,error=FALSE, fig.align='center', fig.height=4,results='hide'}
