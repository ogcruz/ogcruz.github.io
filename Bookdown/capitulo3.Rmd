...
![](~/Documentos/cursos_ecologicos_2019/figuras/LOESS1.png){width=700px}
...
![](~/Documentos/cursos_ecologicos_2019/figuras/running-eps-converted-to.png){width=700px}


![](~/Documentos/cursos_ecologicos_2019/figuras/mediamovel2019.png){width=1270px}


**Exemplo:** Utilizando a ST mensal referente as mortes por Doenças Pulmonares no Reino Unido (1974–1979), temos:
...
$$\hat{f_h}(x) = \dfrac{1}{Nh}\sum{K \left( \dfrac{x-x_i}{h}\right)}$$
  ...
  - $h$ $\textrightarrow$ largura de banda

  - $K$ $\textrightarrow$ função de suavização
...
![](~/Documentos/cursos_ecologicos_2019/figuras/variosKernels-eps-converted-to.png){width=700px}
...
lepto <- read.csv2("leptoruido.csv", header=T)
dia <-  read.table("climadia.dat", header=T)
...
lines(predict(loess(totrain ~ week , data=lepto,span = 0.25, degree = 2)),col="darkblue")
lines(predict(loess(totrain ~ week , data=lepto,span = 0.25, degree = 1)),col="darkblue",lwd=2,lty=2)
legend("topright",c("75% - 2o.","75% - Linear (default)","25% - 2o.","25% - Linear"),lty=c(1,2,1,2), col=c(2,2,"darkblue","darkblue"), title="Janela e Grau",bty="n")
...
![Nearest neighbor smoother](~/Documentos/cursos_ecologicos_2019/figuras/suaveEX1.png){width=700px}
    
![Kernel average smoother](~/Documentos/cursos_ecologicos_2019/figuras/suaveEX2.png){width=7000px}


![Local linear regression](~/Documentos/cursos_ecologicos_2019/figuras/suaveEX3.png){width=7000px}
